...
type LabTrack = "ETHICS" | "DEFENDER" | "EXECUTE" | "INTEL";
...
const tracks: { title: string; track: LabTrack; description: string }[] = [
  {
    ...
    track: "ETHICS",
    description:
      "Design, test, and present an AI-focused portfolio project.",
  },
  {
    title: "Build Cybersecurity Project",
    track: "DEFENDER",
    description:
      "Investigate real scenarios and produce structured security outputs.",
  },
  {
    title: "Build Coding Project",
    track: "EXECUTE",
    description:
      "Build practical software with planning, implementation, and reflection.",
  },
  {
    title: "Build Advanced Project",
    track: "INTEL",
    description:
      "Complete deeper investigations and publish advanced portfolio work.",
  },
];
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4 md:px-8">
          <div>
            ...
            <p className="text-[15px] text-zinc-500">
              Structured portfolio project platform
            </p>
          </div>
      ...
      <main className="mx-auto max-w-6xl px-6 py-12 md:px-8 md:py-16 space-y-14">
        <section className="space-y-6">
          ...
          <p className="text-[15px] leading-7 text-zinc-300 max-w-3xl">
            Create structured AI, cybersecurity, and coding projects that
        ...
        <section className="rounded-xl border border-zinc-800 bg-zinc-950 p-6">
          <div className="grid gap-4 md:grid-cols-3">
            <div>
              <p className="text-[15px] text-zinc-500">Project Type</p>
              <p className="mt-2 text-[26px] font-semibold text-white">
                Structured
              </p>
            </div>
            <div>
              <p className="text-[15px] text-zinc-500">Focus</p>
              <p className="mt-2 text-[26px] font-semibold text-white">
                Portfolio-Ready
              </p>
            </div>
        ...
        <section id="tracks" className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {tracks.map((track) => (
              <article
                key={track.track}
                className="rounded-xl border border-zinc-800 bg-zinc-950 p-6 space-y-4"
              >
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-950 p-6 md:p-8">
            <ol className="grid gap-4 md:grid-cols-4">
              {steps.map((step, i) => (
                <li
                  key={step}
                  className="rounded-lg border border-zinc-800 bg-black px-4 py-4"
                >
